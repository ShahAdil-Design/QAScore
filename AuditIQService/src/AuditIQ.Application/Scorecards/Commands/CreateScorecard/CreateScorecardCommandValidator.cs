...
public sealed class CreateScorecardCommandValidator : AbstractValidator<CreateScorecardCommand>
{
    public CreateScorecardCommandValidator()
    {
        ...
        // The true denominator for the scoring formula (Scorebuddy's own "max score" per
        // version) — required for every scorecard created going forward so native AuditIQ
        // scoring matches Scorebuddy's rather than the coincidental weight-sum approximation.
        RuleFor(c => c.MaxScore).GreaterThan(0);

        // Empty is allowed — a scorecard can be created as a shell first, questions added
        // later via Update (the builder's two-step flow: New Scorecard modal, then Edit).
        RuleForEach(c => c.Questions).ChildRules(question =>
        {
            ...
            question.RuleForEach(q => q.AnswerOptions).ChildRules(answer =>
            {
                ...
                // Convention: a Fail Section / Fail All / Not Applicable option always scores
                // zero — the cascade already forces the effective contribution to zero, but a
                // non-zero configured Value here would misrepresent what the option means.
